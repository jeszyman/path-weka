...
We will use the CBC data from last week's data mining and pre-processing lecture. _The .csv file was emailed to you (NEW-cbc.csv)_. All of these CBC results have normal hemoglobin and hematocrit, but half of the patients will be anemic on their next CBC. The subsAnemia column in the .csv file identifies which patients will be anemic on their next CBC.
